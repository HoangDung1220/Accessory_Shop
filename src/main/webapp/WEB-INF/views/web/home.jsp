...
<div class="well well-small">
	<ul class="nav nav-list">
		<c:forEach var ="item" items="${categories}">
			<li><a href='<c:url value="/san-pham/${item.id }/${1}/${10}" />'><span class="icon-chevron-right"></span> ${item.name}</a></li>
		</c:forEach>
		...
		<li> <a class="totalInCart" href="cart.html"><strong>Total Amount  <span class="badge badge-warning pull-right" style="line-height:18px;">$448.42</span></strong></a></li>
			...
			<li>
			  <div class="thumbnail">
				<a class="zoomTool" href="/detail/${item.id }" title="add to cart"><span class="icon-search"></span> QUICK VIEW</a>
...
-->
            ...
            <c:if test="${ newProduct.size() > 0 }">
						<div class="item active">
							...
							<c:forEach var="item" items="${ newProduct }" varStatus="loop">
								<li class="span3">
									<div class="thumbnail">
										<a class="zoomTool" href="/detail/${item.id }"
											title="add to cart"><span class="icon-search"></span>
											QUICK VIEW</a> <a href='<c:url value ="/detail/${item.id }"/>'><img
											src="<c:url value="/template/web/img/${ item.img }"/>" alt=""></a>
									</div>
								</li>
	...
	<div class="well well-small">
		  ...
		  <div class="row-fluid">
		  <ul class="thumbnails">
		  <c:forEach var="item" items="${highProduct}" varStatus="loop">
		  <li class="span4">
			  <div class="thumbnail">
				<a class="zoomTool" href="/detail/${item.id }" title="add to cart"><span class="icon-search"></span> QUICK VIEW</a>
				<a href='<c:url value ="/detail/${item.id }"/>'><img width="1000" src="<c:url value='/template/web/img/${item.img}'/> " alt=""></a>
				<div class="caption">
				  <h5>${item.name}</h5>
				  <h4>
			...
			<c:if test="${ (loop.index + 1) % 3 == 0 || (loop.index + 1)  == highProduct.size() }">
			</ul>
			<c:if test="${(loop.index+1)< highProduct.size()}">
			<ul class="thumbnails">
			</c:if>
			</c:if>  
